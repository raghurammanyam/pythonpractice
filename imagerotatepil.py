...
def main():
    img = cv2.imread('/home/caratred/Downloads/VALERIE BEATRICE_MOGLIA_347.jpeg')
    cnt = np.array([
            [[124, 49]],
            [[122, 11]],
            [[391, 326]],
    ...
    new_size = (int(img_rot.shape[1]), int(img_rot.shape[0]))
    img_rot_resized = cv2.resize(img_rot, new_size)
    new_size = (int(img.shape[1])), int(img.shape[0])
